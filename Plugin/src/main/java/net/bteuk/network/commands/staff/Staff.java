...
public class Staff extends AbstractCommand {
    ...
    public void execute(@NotNull CommandSourceStack stack, String @NotNull [] args) {
        ...
        NetworkUser user = guiProvider.instance().getUser(p);
        ...
        if (!(user.hasPermission("uknet.staff"))) {
        ...
        if ((args.length > 0 && guiProvider.constants().staffChat())) {
            ...
            if (args[0].equalsIgnoreCase("chat")) {
                ...
                if (user.getChatChannel().equals(STAFF.getChannelName())) {
                    user.player.sendMessage(ChatUtils.success("Disabled staff chat."));
                } else {
                    // Set the chat channel to staff.
                    channel = STAFF.getChannelName();
                    user.player.sendMessage(ChatUtils.success("Enabled staff chat."));
                }
                // Set channel.
                ...
                user.setChatChannel(STAFF.getChannelName());
                guiProvider.chatAPI().sendChatMessage(CustomChat.getChatMessage(Component.text(String.join(" ", args)), user));
                user.setChatChannel(GLOBAL.getChannelName());
            }
        }
