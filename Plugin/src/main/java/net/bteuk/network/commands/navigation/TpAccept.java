...
public class TpAccept extends AbstractCommand {

    private final Network instance;
    private final MessageSender messageSender;

    public TpAccept(Network instance, MessageSender messageSender) {
    ...
    public void execute(@NonNull CommandSourceStack stack, String @NonNull [] args) {
        ...
        // Check if args exist.
        if (args.length == 0) {
            player.sendMessage(ChatUtils.error("/tpaccept <player>"));
            return;
        }
        ...
        TeleportEvent teleportEvent = new TeleportEvent(optionalRequester.get().getUuid(), player.getUniqueId().toString(), TeleportRequestType.ACCEPT);
